#!/usr/bin/env python3
"""
camera_distortion.py – Postprocesador de distorsión de lente Brown-Conrady
===========================================================================
El renderizador Ogre2 de Gazebo Harmonic ignora silenciosamente el elemento
<distortion> del SDF. Este nodo aplica el mismo modelo Brown-Conrady (Plumb Bob)
como paso de postproceso en ROS 2 usando remap con numpy puro.

Pipeline
--------
  Gazebo (render limpio)
    → /camera/image_raw           (sensor_msgs/Image)
    → /camera/image_raw/camera_info   (sensor_msgs/CameraInfo)
                ↓  este nodo
    → /camera/image_distorted     (sensor_msgs/Image)

Cómo se aplica la distorsión
-----------------------------
Para cada posición de píxel de salida (xd, yd) se encuentra la posición fuente
no distorsionada correspondiente (xu, yu) invirtiendo el modelo Brown-Conrady
de forma iterativa, luego se muestrea la imagen limpia de Gazebo ahí con
interpolación bilineal pura numpy:

    output_distorted[yd, xd] = clean_gazebo[yu, xu]

Las tablas de remap se calculan una vez con el primer mensaje camera_info
y se cachean para todos los fotogramas posteriores.

Parámetros
----------
  k1, k2, k3   float   Coeficientes de distorsión radial
  p1, p2        float   Coeficientes de distorsión tangencial
  input_topic   str     Topic de imagen sin procesar  (default: /camera/image_raw)
  output_topic  str     Imagen distorsionada  (default: /camera/image_distorted)
  info_topic    str     Topic CameraInfo (default: <input_topic>/camera_info)
"""
...
# ---------------------------------------------------------------------------
# Conversión ROS Image ↔ numpy  (sin cv_bridge ni cv2)
# ---------------------------------------------------------------------------
...
def _numpy_to_imgmsg(arr: np.ndarray, encoding: str, header) -> Image:
    msg          = Image()
    msg.header   = header
...
# ---------------------------------------------------------------------------
# Remap bilineal y undistort iterativo — sin cv2
# ---------------------------------------------------------------------------

def _undistort_points(pts: np.ndarray, K: np.ndarray, D: np.ndarray) -> np.ndarray:
    """
    Invierte el modelo Brown-Conrady de forma iterativa (equivalente a
    cv2.undistortPoints con P=K).

    pts : (N, 2)  coordenadas de píxel distorsionadas
    K   : (3, 3)  matriz intrínseca
    D   : (5,)    [k1, k2, p1, p2, k3]
    Devuelve (N, 2) coordenadas de píxel no distorsionadas.
    """
    fx, fy = K[0, 0], K[1, 1]
    cx, cy = K[0, 2], K[1, 2]
    k1, k2, p1, p2, k3 = D
    ...
    # Inversión iterativa (5 iteraciones son suficientes)
    x0, y0 = x.copy(), y.copy()
    for _ in range(5):
        r2      = x * x + y * y
...
def _remap_bilinear(src: np.ndarray,
                    map_x: np.ndarray,
                    map_y: np.ndarray) -> np.ndarray:
    """Interpolación bilineal con BORDER_CONSTANT=0 fuera del borde."""
    h, w = src.shape[:2]
    ...
    result = ((1 - wy) * ((1 - wx) * p00 + wx * p01)
              +     wy  * ((1 - wx) * p10 + wx * p11)).astype(src.dtype)
...
class CameraDistortionNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'Distorsión Brown-Conrady activa — '
    ...
    def _info_cb(self, msg: CameraInfo):
        ...
        self.get_logger().debug(
            f'Tablas de remap construidas — {w}×{h} '
